...
const UI = {
    ...
    start() {
        ...
        UI.addClickHandle("noVNC_audio_button", UI.toggleAudio);
        UI.showControlInput("noVNC_audio_button");
 ...
 * ------v------*/
    ...
    * ------v------*/
    ...
    toggleAudio() {
        if(UI.audioInfo === null){
            document.getElementById('noVNC_audio_button').classList.add("noVNC_selected");
            UI.showStatus('Audio enabled.', 'warn', 2000, true);

            let continueFlag = true;

            let attemptConnect = async () => {
                let audioCtx = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: 32000 });
                await audioCtx.audioWorklet.addModule('/pcspk-sound/worklet-processor.js');

                let workletNode = new AudioWorkletNode(audioCtx, 'pcm-stream-processor');
                workletNode.connect(audioCtx.destination);

                let port = workletNode.port;
                port.onmessage = (event) => {
                    if (event.data.type === 'chunk') {
                        console.log(`Received audio chunk: ${event.data.size} samples`);
                    }
                };

                const response = await fetch('/pcspk-sound/stream.wav');
            ...
            let waitProm = new Promise(async () => {
                while(continueFlag){
                    try {
            ...
            UI.audioInfo.cancel();
            UI.audioInfo = null;
            document.getElementById('noVNC_audio_button').classList.remove("noVNC_selected");
            UI.showStatus('Audio disabled.', 'warn', 2000, true);
        }
    },
